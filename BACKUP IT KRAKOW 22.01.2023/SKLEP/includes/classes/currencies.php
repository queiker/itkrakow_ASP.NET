...
//TotalB2B start
    function display_price($products_id, $products_price, $products_tax, $quantity = 1) {
	  ...
	  if(defined('ALLOW_GUEST_TO_SEE_PRICES')) {
		  $query_price_to_guest_result['configuration_value'] = ALLOW_GUEST_TO_SEE_PRICES;
	  } else {
      ...
      if (($query_price_to_guest_result['configuration_value']=='true') && !(tep_session_is_registered('customer_id'))) {
		 if(defined('GUEST_DISCOUNT')) {
				$query_guest_discount_result['configuration_value'] = GUEST_DISCOUNT;
		 } else {
				$query_guest_discount = tep_db_query("select configuration_value from " . TABLE_CONFIGURATION . " WHERE configuration_key = 'ALLOW_GUEST_TO_SEE_PRICES'");
				$query_guest_discount_result = tep_db_fetch_array($query_price_to_guest); 
		 }
