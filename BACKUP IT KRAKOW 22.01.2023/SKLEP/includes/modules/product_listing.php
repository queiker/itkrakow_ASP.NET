...
<!-- product_listing_col //-->
<?php
if((isset($view) && $view == 'slim')) {
	include(DIR_WS_MODULES . FILENAME_PRODUCT_LISTING_SLIM);
} else if(isset($view) && $view == 'box') {
	include(DIR_WS_MODULES . FILENAME_PRODUCT_LISTING_BOXED);
} else {
  $producenci = array();
...
//$lc_text .= '&nbsp;</td></tr>';
      if (PRODUCT_LIST_BUY_NOW) {
		...
		$lc_text .= '<td align="center" width="150" valign="middle" style="border-top: 5px solid #FFF;" NOWRAP><span class="cenaBig">'.$cena.'</span><br /><br />
			<div class="btn"><a href="' . tep_href_link(basename($PHP_SELF), 'products_id=' . (int)$listing_values['products_id'].'&action=add_wishlist', 'NONSSL') . '" title="Dodaj do schowka">' . tep_image_button('button_wishlist.gif',  'Dodaj do schowka '. $listing_values['products_name'] ) . '</a></div>
			<div class="btn"><a href="' . tep_href_link(basename($PHP_SELF), 'products_id=' . $listing_values['products_id'].'&amp;action=buy_now', 'NONSSL') . '">' . tep_image_button('button_in_cart.gif', TEXT_BUY . $listing_values['products_name'] . TEXT_NOW,' style="vertical-align: middle;"') . '</a></div></td></tr></table>';
      }
...
$lc_text .= '</table></div><br />';
    ...
    $info_box_contents = array();
	echo '<div class="blrb" align="center">'.TEXT_NO_PRODUCTS.'</div>';
