    ...
    function dp() {
      ...
      if ( ($this->enabled == true) && ((int)MODULE_SHIPPING_DP_ZONE > 0) ) {
		...
		if(isset($cart)) {
			...
			if ($paczka > 0 && $check_flag == true) $this->enabled = ((MODULE_SHIPPING_DP_STATUS == 'True') ? true : false);
		}
...
// class methods
      ...
      if ($dest_zone == 0) {
        ...
        for ($i=0; $i<sizeof($dp_table); $i+=2) {
          if ($shipping_weight <= $dp_table[$i]) {
            ...
            $shipping_method = MODULE_SHIPPING_DP_TEXT_WAY . ' ';
            break;
          }
        }
    ...
    function install() {
      tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added) VALUES ('Poczta Polska', 'MODULE_SHIPPING_DP_STATUS', 'True', 'Czy chcesz oferowaæ wysy³kê za po¶rednictwem Poczty Polskiej?', '6', '0', 'tep_cfg_select_option(array(\'True\', \'False\'), ', now())");

	  tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Koszt pakowania', 'MODULE_SHIPPING_DP_HANDLING', '0', '', '6', '0', now())");

	  tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, use_function, set_function, date_added) values ('Stawka podatkowa', 'MODULE_SHIPPING_DP_TAX_CLASS', '0', '', '6', '0', 'tep_get_tax_class_title', 'tep_cfg_pull_down_tax_classes(', now())");

	  tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, use_function, set_function, date_added) values ('Z jakiego kraju wysy³ka', 'MODULE_SHIPPING_DP_ZONE', '0', '', '6', '0', 'tep_get_zone_class_title', 'tep_cfg_pull_down_zone_classes(', now())");

	  tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Kolejno¶æ wy¶wietlania', 'MODULE_SHIPPING_DP_SORT_ORDER', '0', 'Kolejno¶æ wy¶wietlania. Najni¿sze wy¶wietlane s± na pocz±tku.', '6', '0', now())");

	  tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Kraj docelowy', 'MODULE_SHIPPING_DP_COUNTRIES_1', 'PL', 'Oddzielana przecinkami lista kodów ISO krajów', '6', '0', now())");

	  tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Stawki za poszczególne paczki', 'MODULE_SHIPPING_DP_COST_1', '1:6.5,2:8,5:9.5,10:15,15:19,20:27,30:30', 'Wpisz wagê oraz odpowiadaj±cy jej koszt przesy³ki, np. <b>3:8.50,5:10</b> . Do 3kg wysy³ka 8.50PLN. 3 do 5kg wysylka 10PLN', '6', '0', now())");
    }
