...
// define the project version
  define('PROJECT_VERSION', 'Mysklep Comfort 2.5 [@osC 2.2-MS2]');
...
// make a connection to the database... now
  tep_db_connect() or die('Nie mozna polaczyc sie z baza danych!');
...
//Wishlist actions (must be before shopping cart actions)
  if(isset($HTTP_POST_VARS['wishlist_x'])) {
	if(isset($HTTP_POST_VARS['products_id'])) {
		...
		$wishlist_id = (int)tep_db_prepare_input($HTTP_POST_VARS['products_id']);
		if($wishlist_id < 1) {
  ...
  if(tep_db_prepare_input($HTTP_GET_VARS['action']) == 'move_wishlist') {
	if(isset($HTTP_GET_VARS['products_id'])) {
		...
		$wishlist_id = '';
		tep_session_register('wishlist_id');
		$wishlist_id = (int)tep_db_prepare_input($HTTP_GET_VARS['products_id']);
		if($wishlist_id < 1) {
			tep_session_unregister('wishlist_id');		
		}
	}
...
// redirect the customer to a friendly cookie-must-be-enabled page if cookies are disabled
    ...
    switch ($HTTP_GET_VARS['action']) {
      ...
      case 'cust_order' :     if (tep_session_is_registered('customer_id') && isset($HTTP_GET_VARS['pid'])) {
                              ...
                              tep_redirect(tep_href_link($goto, tep_get_all_get_params($parameters)));
                              break;
      case 'add_wishlist' :   if (isset($HTTP_GET_VARS['products_id'])) {
                                if (tep_has_product_attributes($HTTP_GET_VARS['products_id'])) {
                                  tep_redirect(tep_href_link(FILENAME_PRODUCT_INFO, 'products_id=' . $HTTP_GET_VARS['products_id']));
                                } else {
									$wishlist_id = '';
      ...
      case 'remove_wishlist' :
								if (isset($HTTP_GET_VARS['products_id'])) {
									$produkt = tep_db_prepare_input($HTTP_GET_VARS['products_id']);
									$wishList->remove($produkt);	
								}
								tep_redirect(tep_href_link($goto, tep_get_all_get_params($parameters)));
								break;
    }
...
// add category names or the manufacturer name to the breadcrumb trail
  if (isset($cPath_array)) {
    ...
    $manufacturers_query = tep_db_query("select manufacturers_name from " . TABLE_MANUFACTURERS . " where manufacturers_id = '" . (int)tep_db_prepare_input($HTTP_GET_VARS['manufacturers_id']) . "'");
    if (tep_db_num_rows($manufacturers_query)) {
      $manufacturers = tep_db_fetch_array($manufacturers_query);
      $breadcrumb->add($manufacturers['manufacturers_name'], tep_href_link(FILENAME_DEFAULT, 'manufacturers_id=' . (int)tep_db_prepare_input($HTTP_GET_VARS['manufacturers_id'])));
    }
  }
...
// add the products model to the breadcrumb trail
  if (isset($HTTP_GET_VARS['products_id'])) {
    ...
    if (tep_db_num_rows($model_query)) {
      ...
      $breadcrumb->add($model['products_model'], tep_href_link(FILENAME_PRODUCT_INFO, 'cPath=' . $cPath . '&products_id=' . (int)$HTTP_GET_VARS['products_id']));
    }
  }
