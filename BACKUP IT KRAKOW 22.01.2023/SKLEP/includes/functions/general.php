...
/*
  ...
  v	1.234 - sprawdzanie ID produktu - BF1617
  v BF - HTTP Header Injection
  v BF - Adding Non-Existing Products To Cart
...
// Return a random row from a database query
  ...
  function tep_get_products_tax($product_id) {
    $product_query = tep_db_query("select products_tax_class_id from " . TABLE_PRODUCTS . " where products_id = '" . (int)$product_id . "'");
...
// TABLES: products
  ...
  function tep_get_products_desc($product_id, $language = '') {
    global $languages_id;
...
function tep_get_tax_rate($class_id, $country_id = -1, $zone_id = -1) {
	...
	if(tep_not_null($customer_zone_id)) $zone_id = $customer_zone_id;
	if(tep_not_null($customer_country_id)) $country_id = $customer_country_id;
...
// TABLES: address_format
  function tep_address_format($address_format_id, $address, $html, $boln, $eoln) {
    ...
    if (isset($address['nip']) && tep_not_null($address['nip'])) {    
		$nip = 'NIP: ';
    ...
    $street_address_dom = tep_output_string_protected($address['street_address_dom']);
    $street_address_mieszkanie = tep_output_string_protected($address['street_address_mieszkanie']);
...
// Text Mode
    ...
    $statecomma = '';
    $streets = $street;
    if ($street_address_dom != '') $streets = $streets . ' ' . $street_address_dom;
    if ($street_address_mieszkanie != '') $streets = $streets . '/' . $street_address_mieszkanie;
    if ($suburb != '') $streets = $streets . $cr . $suburb;

    if ($state != '') $statecomma = $state . ', ';
    if ($country == '' || strlen($country) < 2) $country = tep_output_string_protected($address['country']);
    ...
    if ( (ACCOUNT_NIP == 'true') && (tep_not_null($nip)) ) {
      $address = $address . $cr . $nip;
    }
...
// TABLES: customers, address_book
  function tep_address_label($customers_id, $address_id = 1, $html = false, $boln = '', $eoln = "\n") {
    $address_query = tep_db_query("select entry_firstname as firstname, entry_lastname as lastname, entry_company as company, entry_nip as nip, entry_street_address as street_address, entry_street_address_dom as street_address_dom, entry_street_address_mieszkanie as street_address_mieszkanie, entry_suburb as suburb, entry_city as city, entry_postcode as postcode, entry_state as state, entry_zone_id as zone_id, entry_country_id as country_id from " . TABLE_ADDRESS_BOOK . " where customers_id = '" . (int)$customers_id . "' and address_book_id = '" . (int)$address_id . "'");
...
// Check date
  function tep_checkdate($date_to_check, $format_string, &$date_array) {
    ...
    $separators = array('-', ' ', '/', '.');
    $month_abbr = array('st','lut','mar','kwie','maj','czerw','lip','sier','wrz','paz','list','gru');
    ...
    if ($date_separator_idx != -1) {
      ...
      for ($i=0; $i<$size; $i++) {
        if ($format_string_array[$i] == 'mm' || $format_string_array[$i] == 'mmm') $month = $date_to_check_array[$i]; //$month ="fsd";
...
// Return table heading with sorting capabilities
  function tep_create_sort_heading($sortby, $colnum, $heading) {
    ...
    if ($sortby) {
      ...
      $sort_suffix = (substr($sortby, 0, 1) == $colnum ? (substr($sortby, 1, 1) == 'a' ? '<img src="images/sort_arrow_up.gif" width="10" height="10" border="0">' : '<img src="images/sort_arrow_down.gif" width="10" height="10" border="0">') : '') . '</a>';
    }
...
// >>> BEGIN REGISTER_GLOBALS
  ...
  function link_get_variable($var_name)
  {
    ...
    {
      $GLOBALS[$var_name] =& $HTTP_GET_VARS[$var_name];
    }
  }
  ...
  function link_post_variable($var_name)
  {
    ...
    {
      $GLOBALS[$var_name] =& $HTTP_POST_VARS[$var_name];
    }
  }
...
// Return table heading with sorting capabilities
  function tep_create_sort_pulldown($sortby, $colnum_list) {
    global $PHP_SELF, $column_list;
  ...
  for ($col=0, $n=sizeof($column_list); $col<$n; $col++) {
    switch ($column_list[$col]) {
      ...
      case 'PRODUCT_LIST_BUY_NOW':
        ...
        break;
	  default:
        $lc_text = '+'.$column_list[$col];
        $lc_align = 'center';
        break;
    }

    if ( ($column_list[$col] != 'PRODUCT_LIST_BUY_NOW') && ($column_list[$col] != 'PRODUCT_LIST_IMAGE') && ($column_list[$col] != 'PRODUCT_LIST_MODEL')) {	
        $sort_range[] = array('id' => ($col+1) . 'a', 'text' => $lc_text . TEXT_PRODUKTU . ' - ' . TEXT_ASCENDINGLY);
        $sort_range[] = array('id' => ($col+1) . 'd', 'text' => $lc_text . TEXT_PRODUKTU . ' - ' . TEXT_DESCENDINGLY);
	}
  }
