...
pa.options_values_id = '" . $products_attributes[0]['value_id'] . "'";
        ...
        $stock_query = tep_db_query("select products_quantity from " . TABLE_PRODUCTS . " where products_id = 
...
products_id = '" . tep_get_prid($order->products[$i]['id']) . "'");
        if ( ($stock_left < 1) && (STOCK_ALLOW_CHECKOUT == 'false') ) {
          tep_db_query("update " . TABLE_PRODUCTS . " set products_status = '0' where products_id = '" . 
tep_get_prid($order->products[$i]['id']) . "'");
        }
      }
    }
...
pa.options_values_price, pa.price_prefix, pad.products_attributes_maxdays, pad.products_attributes_maxcount , 
...
$order->products[$i]['id'] . "' and pa.options_id = '" . $order->products[$i]['attributes'][$j]['option_id'] . 
"' and pa.options_id = popt.products_options_id and pa.options_values_id = '" . 
$order->products[$i]['attributes'][$j]['value_id'] . "' and pa.options_values_id = 
...
if (!tep_session_is_registered('noaccount')) 
{
 ...
 if (is_object($$payment)) {
   ...
   //tu sprawdzamy, czy klient wybra³ mtransfer, i dodajemy link do p³atno¶æi w mailu z info o zakupie.
   //deklaracja zmiennej zawieraj±cej dane o transakcji wysy³ane ze sklepu do mbanku
   $nr_zamowienia = ' nr zamowienia: ';
   $Description =  $Description.$nr_zamowienia;
   $Description = ereg_replace(" ","_",$Description);
   $Amount = ereg_replace(',', '', $Amount);
   $mtransfer_mail= 'https://www.mbank.com.pl/mtransfer.asp?osCsid='.$osCsid.'&ServiceID='.$ServiceID.'&Amount='.$Amount.'&TrDate='.$TrDate.'&Description='.$Description.$insert_id;
   $payment_class = $$payment;
   $email_order .= $order->info['payment_method'] . "\n\n";
   if ($TrDate != "") {
		$email_order .= MBANK_EMAIL_INFO . "\n" . $mtransfer_mail . "\n\n";
   } else {
...
tep_session_register('last_order');
  ...
  tep_mail($order->customer['firstname'] . ' ' . $order->customer['lastname'], $order->customer['email_address'], EMAIL_TEXT_SUBJECT .' ['.$oID.']', $email_order, STORE_OWNER, STORE_OWNER_EMAIL_ADDRESS);
...
//extra info o kliencie
  ...
  if ($order->customer['fax'] != '') {$email_order .= "\n" . FAX_GSM_EMAIL_INFO . $order->customer['fax'] . "\n\n";};
   ...
   tep_mail('Zamówienie ze sklepu', STORE_OWNER_EMAIL_ADDRESS, EMAIL_TEXT_SUBJECT .' ['.$oID.']', $email_order, STORE_OWNER, STORE_OWNER_EMAIL_ADDRESS);

// send emails to other people
  if (SEND_EXTRA_ORDER_EMAILS_TO != '') {
    tep_mail('Zamówienie', SEND_EXTRA_ORDER_EMAILS_TO, EMAIL_TEXT_SUBJECT .' ['.$oID.']', $email_order, STORE_OWNER, STORE_OWNER_EMAIL_ADDRESS);
  }
...
<!DOCTYPE html public "-//W3C//DTD HTML 4.01 Transitional//EN">
  ...
  <input type="hidden" name="TrDate" value="<?= $TrDate ;?>">
  <input type="hidden" name="Description" value="<?= $Description.$insert_id ;?>">
  <input type="image" src="<?php echo DIR_WS_LANGUAGES . $language . '/'; ?>images/buttons/button_confirm_order.gif" border="0" alt="Potwierdz zamówienie" title=" Potwierdz zamówienie ">
  </form>
  </center>
  </body>
  </html>
