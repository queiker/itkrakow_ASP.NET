...
//TotalB2B end
        ...
        $entry_street_address_dom = tep_db_prepare_input($HTTP_POST_VARS['entry_street_address_dom']);
        $entry_street_address_mieszkanie = tep_db_prepare_input($HTTP_POST_VARS['entry_street_address_mieszkanie']);
...
//TotalB2B end
        ...
        $sql_data_array = array('entry_firstname' => $customers_firstname,
                                ...
                                'entry_street_address_dom' => $entry_street_address_dom,
                                'entry_street_address_mieszkanie' => $entry_street_address_mieszkanie,
...
//TotalB2B end
      ...
      default:
//TotalB2B start
        $customers_query = tep_db_query("select c.customers_id, c.customers_gender, c.customers_firstname, c.customers_lastname, c.customers_dob, c.customers_email_address, a.entry_company, a.entry_nip, a.entry_street_address, a.entry_street_address_dom, a.entry_street_address_mieszkanie, a.entry_suburb, a.entry_postcode, a.entry_city, a.entry_state, a.entry_zone_id, a.entry_country_id, c.customers_telephone, c.customers_fax, c.customers_newsletter, c.customers_groups_id, c.customers_discount, c.customers_default_address_id, c.customers_notes from " . TABLE_CUSTOMERS . " c left join " . TABLE_ADDRESS_BOOK . " a on c.customers_default_address_id = a.address_book_id where a.customers_id = c.customers_id and c.customers_id = '" . (int)$HTTP_GET_VARS['cID'] . "'");
//TotalB2B end
...
<?php if (ACCOUNT_DOB == 'true') echo 'var customers_dob = document.customers.customers_dob.value;' . "\n"; ?>
  ...
  var entry_street_address_dom = document.customers.entry_street_address_dom.value;
  var entry_street_address_mieszkanie = document.customers.entry_street_address_mieszkanie.value;
...
<?php } ?>
  ...
  if (entry_street_address_dom.length < <?php echo ENTRY_STREET_ADDRESS_DOM_MIN_LENGTH; ?>) {
    error_message = error_message + "<?php echo JS_ADDRESS_DOM; ?>";
    error = 1;
  }

  if (entry_street_address_mieszkanie.length < <?php echo ENTRY_STREET_ADDRESS_MIESZKANIE_MIN_LENGTH; ?>) {
    error_message = error_message + "<?php echo JS_ADDRESS_DOM; ?>";
    error = 1;
  }
...
<!-- Insert Pazio pazio@sitenet.pl start -->
          <tr>
...
<!-- Insert Pazio pazio@sitenet.pl end -->
...
<?php
  if ($error == true) {
    if ($entry_street_address_error == true) {
      echo tep_draw_input_field('entry_street_address_dom', $cInfo->entry_street_address_dom, 'maxlength="64"') . '&nbsp;' . ENTRY_STREET_ADDRESS_ERROR;
    } else {
      echo $cInfo->entry_street_address_dom . tep_draw_hidden_field('entry_street_address_dom');
    }
  } else {
    echo tep_draw_input_field('entry_street_address_dom', $cInfo->entry_street_address_dom, 'maxlength="64"', true);
  }
?></td>
          </tr>
...
<?php
  if ($error == true) {
    if ($entry_street_address_error == true) {
      echo tep_draw_input_field('entry_street_address_mieszkanie', $cInfo->entry_street_address_mieszkanie, 'maxlength="64"') . '&nbsp;' . ENTRY_STREET_ADDRESS_ERROR;
    } else {
      echo $cInfo->entry_street_address_mieszkanie . tep_draw_hidden_field('entry_street_address_mieszkanie');
    }
  } else {
    echo tep_draw_input_field('entry_street_address_mieszkanie', $cInfo->entry_street_address_mieszkanie, 'maxlength="64"', true);
  }
?></td>
          </tr>
...
?>
          <tr>
            <td valign="top" class="main">Notatki:</td>
            <td class="main">
<?php
  if ($processed == true) {
  ...
  switch ($action) {
    ...
    default:
      if (isset($cInfo) && is_object($cInfo)) {
        ...
        $contents[] = array('text' => '<strong>'.ENTRY_CUSTOMERS_NOTES.'</strong><br><br>' . $cInfo->customers_notes);
      }
      break;
  }
