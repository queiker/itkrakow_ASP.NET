...
$download = tep_db_prepare_input($HTTP_GET_VARS['download']);
$dltype = tep_db_prepare_input($HTTP_GET_VARS['dltype']);
$split = tep_db_prepare_input($HTTP_GET_VARS['split']);
...
$MAX_FILE_SIZE = tep_db_prepare_input($HTTP_POST_VARS['MAX_FILE_SIZE']);
$buttoninsert = tep_db_prepare_input($HTTP_POST_VARS['buttoninsert']);
$buttonsplit = tep_db_prepare_input($HTTP_POST_VARS['buttonsplit']);
$localfile = tep_db_prepare_input($HTTP_POST_VARS['localfile']);
...
header("Pragma: ");
		...
		fclose($fp);
		echo "Mo¿esz pobraæ pliki z dzia³u Narzêdzia --> Ekslorator Plików jako: " . $tempdir . "EP" . $EXPORT_TIME . ".txt";
		die();
	}
...
echo "Easy Populate $curver - Domy¶lny jêzyk : " . $epdlanguage_name . '(' . $epdlanguage_id .')' ;
?>
...
if (is_uploaded_file($usrfl) && $split==1) {
	...
	echo "Tworzenie pliku EP_Split" . $filecount . ".txt ...  ";
	...
	while ($line){
		...
		if (strpos($line, 'EOREOR')){
			...
			if ($linecount >= $maxrecs){
				echo "Dodano $linecount rekordów i zamkniêto plik... <Br>";
	...
	echo "Dodano $linecount rekordów i zamkniêto plik...<br><br> ";
	fclose($fp);
	fclose($infp);

	echo "Mo¿esz ¶±ci±gn±æ pliki EP w Narzêdzia/Eksplorator plików z /admin/ep";
...
?>
      ...
      <table width="75%" border="2">
        <tr>
          <td width="75%">
           ...
           <FORM ENCTYPE="multipart/form-data" ACTION="easypopulate.php" METHOD=POST>
              <p>
                <div align = "left">
                <p><b>Import z Katalogu Tymczasowego (<?php echo $tempdir; ?>)</b></p>
		<p class="smallText">
		...
		<p><b>¦ci±gnij pliki EP i Froogle</b></p>

	      <!-- Download file links -  Add your custom fields here -->
	  <a href="easypopulate.php?download=stream&dltype=full">¦ci±gnij <b>Kompletny</b> plik .txt (rozdzielany tabulatorem) do edycji</a><br>
	  <a href="easypopulate.php?download=stream&dltype=priceqty">¦ci±gnij <b>Model/Cena/Ilo¶æ</b> plik .txt (rozdzielany tabulatorem) do edycji</a><br>
	  <a href="easypopulate.php?download=stream&dltype=category">¦ci±gnij <b>Model/Kategoria</b> plik .txt (rozdzielany tabulatorem) do edycji</a><br>
	  <a href="easypopulate.php?download=stream&dltype=froogle">¦ci±gnij <b>Froogle</b> plik .txt (rozdzielany tabulatorem) do edycji</a><br>

			<!-- VJ product attributes begin //-->
<?php
  if ($products_with_attributes == true) {
?>
	  <a href="easypopulate.php?download=stream&dltype=attrib">¦ci±gnij <b>Model/Cechy</b> plik .txt (rozdzielany tabulatorem) do edycji</a><br>
<?php
  }
?>
			<!-- VJ product attributes end //-->

		<p><b>Zapisz pliki EP and Froogle w katalogu tymczasowym (<?php echo $tempdir; ?>)</b></p>
	  <a href="easypopulate.php?download=tempfile&dltype=full">Tworzenie pliku .txt (rozdzielany tabulatorem) w katalogu tymczasowym - Kompletny</a><br>
          <a href="easypopulate.php?download=tempfile&dltype=priceqty"">Tworzenie pliku .txt (rozdzielany tabulatorem) w katalogu tymczasowym - Model/Cena/Ilo¶æ</a><br>
          <a href="easypopulate.php?download=tempfile&dltype=category">Tworzenie pliku .txt (rozdzielany tabulatorem) w katalogu tymczasowym - Model/Kategoria</a><br>
	  <a href="easypopulate.php?download=tempfile&dltype=froogle">Tworzenie pliku .txt (rozdzielany tabulatorem) w katalogu tymczasowym - Froogle</a><br>

			<!-- VJ product attributes begin //-->
	  <a href="easypopulate.php?download=tempfile&dltype=attrib">Tworzenie pliku .txt (rozdzielany tabulatorem) w katalogu tymczasowym - Model/Cechy</a><br>
			<!-- VJ product attributes end //-->
...
//		$v_date_avail = "CURRENT_TIMESTAMP";
	...
	if (strlen($v_products_model) > $modelsize ){
		echo "<font color='red'>" . strlen($v_products_model) . $v_products_model . "... ERROR! - Za du¿o znaków w nazwie modelu.<br>
			Standardow± warto¶ci± dla osCommerce PRO jest 32.<br>
			Twoja warto¶æ maksymalna dla product_model ustawiona jest na $modelsize<br>
			Powiniene¶ albo skróciæ nazwê modelu albo zwiêkszyæ d³ugo¶c tego pola w bazie danych.</font>";
		die();
	}
	...
	if ($v_products_model != "") {
		...
		if (tep_db_num_rows($result) == 0)  {
			...
			echo "<font color='red'><b> !Nowy Produkt! </b></font><br>";
			...
			*/
			...
			echo "<font color='green'> Uaktualniono</font><br>---------------<br>";
			$row =  tep_db_fetch_array($result);
...
*/
		...
		if (isset($v_customer_price_1)){
			...
			if (($v_customer_group_id_1 == '') AND ($v_customer_price_1 != ''))  {
				echo "<font color=red>B³±d - v_customer_group_id i v_customer_price musz± wystêpowaæ jednocze¶nie</font>";
				die();
			}
		...
		array_walk($items, 'print_el');
		echo "<p class=smallText>Nie ma pola products_model w rekordzie. Ta linia nie zosta³a zaimportowana <br>";
		echo "<br>";
	}
