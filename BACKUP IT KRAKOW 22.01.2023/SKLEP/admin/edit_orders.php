  ...
  if (($check_status['orders_status'] != $_POST['status']) || (tep_not_null($_POST['comments']))) {
          ...
          foreach($_POST['update_products'] as $orders_products_id => $products_details) {
			...
			$quantity_difference = ($products_details['qty'] - $order_products['products_quantity']);
				if (STOCK_LIMITED == 'true'){
					...
					// update the options stock as well here: Begin
					// 1. get the attributes from the attroibutes tables.
					// 2. update for each o the quantities
					$attrib_q = tep_db_query("select products_id,products_options, products_options_values from orders_products_attributes opa, orders_products op where op.orders_id = '" . $oID . "' AND op.orders_products_id = '" . $orders_products_id . "' and opa.orders_products_id = op.orders_products_id");
					while($attrib_set = tep_db_fetch_array($attrib_q))
					{
				     // corresponding to each option find the attribute ids ( opts and values id )
                     $opt_ids = tep_db_query("select po.products_options_id, pov.products_options_values_id from products_options po, products_options_values pov, products_options_values_to_products_options po2pov where products_options_values_name = '".tep_db_input($attrib_set['products_options_values'])."' and pov.products_options_values_id = po2pov.products_options_values_id and po.products_options_id = po2pov.products_options_id and products_options_name = '".tep_db_input($attrib_set['products_options'])."'");
					 $opt = tep_db_fetch_array($opt_ids);
					 $products_stock_attributes = $opt['products_options_id'].'-'.$opt['products_options_values_id'];
					 /*
			         $stock_chk_q = tep_db_query("selct * from " . TABLE_PRODUCTS_STOCK . " where products_id=" . $attrib_set['products_id']. " and products_stock_attributes='".$products_stock_attributes."'");
   ...
   tep_db_query("UPDATE " . TABLE_ORDERS . " SET shipping_module = '" . $shipping['id'] . "' WHERE orders_id = '" . (int)$oID . "'");
	...
	case 'email':
		...
		if(SEND_CUSTOMER_CONFIRM_ORDER == 'true'){
			$order->info['order_status'] = DEFAULT_ORDERS_CONFIRM_STATUS_ID;
		}
	   ...
	   $products_ordered .= $order->products[$i]['qty'] . ' x ' . $order->products[$i]['name'] . $products_model . ' = ' . $currencies->format(tep_add_tax($order->products[$i]['final_price'], $order->products[$i]['tax']) * $order->products[$i]['qty'], true, $order->info['currency'], $order->info['currency_value']) . $products_ordered_attributes . "\n";
		...
		//Build the email
	   	 $email_order = STORE_NAME . "\n" . 
						...
						EMAIL_TEXT_ORDER_NUMBER . ' ' . (int)$oID . "\n";
						if(SEND_CUSTOMER_CONFIRM_ORDER == 'true'){
							$token = substr(md5($oID), 2, 7);
							$email_order .= "\n".EMAIL_TEXT_CONFIRM_ORDER . ' ' . tep_catalog_href_link('order_confirm.php', 'order_id=' . $oID.'&tok='.$token, 'SSL') . "\n";
						} else {
							$email_order .= EMAIL_TEXT_INVOICE_URL . ' ' . tep_catalog_href_link(FILENAME_CATALOG_ACCOUNT_HISTORY_INFO, 'order_id=' . (int)$oID, 'SSL') . "\n";
						}
...
<!-- body_text //-->
 ...
 ?>
      ...
      <div id="header">
		  ...
		  <p id="headerTitle" class="pageHeading"><?php echo 'Edycja zamówienia #'.$_GET['oID']; ?></p>
          ...
          <ul>
		    ...
		    <li><?php echo '<a href="' . tep_href_link(FILENAME_ORDERS_INVOICE, 'oID=' . $_GET['oID']) . '" TARGET="_blank">' . tep_image_button('button_invoice.gif', 'Faktura') . '</a>'; ?></li>
        ...
        <table border="0" cellspacing="0" cellpadding="2">
          <tr>
            ...
            <table width="100%" border="0" cellspacing="0" cellpadding="2" style="border: 1px solid #C9C9C9;">
			  ...
			  <tr class="dataTableRow"> 
                <td class="dataTableContent" valign="middle" align="right" nowrap><?php echo ENTRY_NIP; ?></td>
                <td colspan="3" valign="top" class="dataTableContent"><input name="update_customer_nip" size="37" value="<?php echo stripslashes($order->customer['nip']); ?>" <?php if (ORDER_EDITOR_USE_AJAX == 'true') { ?>onChange="updateOrdersField('customers_nip', encodeURIComponent(this.value))"<?php } ?>></td>
              </tr>
              <tr class="dataTableRow"> 
			  ...
			  <tr class="dataTableRow"> 
                <td class="dataTableContent" valign="middle" align="right" nowrap><?php echo ENTRY_STREET_ADDRESS_DOM; ?></td>
                <td colspan="3" valign="top" class="dataTableContent" nowrap><input name="update_customer_street_address_dom" size="37" value="<?php echo stripslashes($order->customer['street_address_dom']); ?>" <?php if (ORDER_EDITOR_USE_AJAX == 'true') { ?>onChange="updateOrdersField('customers_street_address_dom', encodeURIComponent(this.value))"<?php } ?>></td>
              </tr>
			  <tr class="dataTableRow"> 
                <td class="dataTableContent" valign="middle" align="right" nowrap><?php echo ENTRY_STREET_ADDRESS_MIESZKANIE; ?></td>
                <td colspan="3" valign="top" class="dataTableContent" nowrap><input name="update_customer_street_address_mieszkanie" size="37" value="<?php echo stripslashes($order->customer['street_address_mieszkanie']); ?>" <?php if (ORDER_EDITOR_USE_AJAX == 'true') { ?>onChange="updateOrdersField('customers_street_address_mieszkanie', encodeURIComponent(this.value))"<?php } ?>></td>
              </tr>
              <tr class="dataTableRow"> 
            ...
            <table width="100%" border="0" cellspacing="0" cellpadding="0" style="border: 1px solid #C9C9C9;">
              ...
              <tr id="shippingAddressEntry">
                ...
                <table width="100%" cellspacing="0" cellpadding="2">
				  ...
				  <tr class="dataTableRow"> 
                    <td class="dataTableContent" valign="middle" align="right"><?php echo ENTRY_NIP; ?></td>
                    <td colspan="3" valign="top" class="dataTableContent"><input name="update_delivery_nip" size="37" value="<?php echo stripslashes($order->delivery['nip']); ?>" <?php if (ORDER_EDITOR_USE_AJAX == 'true') { ?>onChange="updateOrdersField('delivery_nip', encodeURIComponent(this.value))"<?php } ?>></td>
                  </tr>
                  <tr class="dataTableRow"> 
				  ...
				  <tr class="dataTableRow"> 
                    <td class="dataTableContent" valign="middle" align="right"><?php echo ENTRY_STREET_ADDRESS_DOM; ?></td>
                    <td colspan="3" valign="top" class="dataTableContent"><input name="update_delivery_street_address_dom" size="37" value="<?php echo stripslashes($order->delivery['street_address_dom']); ?>" <?php if (ORDER_EDITOR_USE_AJAX == 'true') { ?>onChange="updateOrdersField('delivery_street_address_dom', encodeURIComponent(this.value))"<?php } ?>></td>
                  </tr>
				  <tr class="dataTableRow"> 
                    <td class="dataTableContent" valign="middle" align="right"><?php echo ENTRY_STREET_ADDRESS_MIESZKANIE; ?></td>
                    <td colspan="3" valign="top" class="dataTableContent"><input name="update_delivery_street_address_mieszkanie" size="37" value="<?php echo stripslashes($order->delivery['street_address_mieszkanie']); ?>" <?php if (ORDER_EDITOR_USE_AJAX == 'true') { ?>onChange="updateOrdersField('delivery_street_address_mieszkanie', encodeURIComponent(this.value))"<?php } ?>></td>
                  </tr>
                  <tr class="dataTableRow"> 
                    <td class="dataTableContent" valign="middle" align="right"><?php echo ENTRY_SUBURB; ?></td>
            ...
            <table width="100%" border="0" cellspacing="0" cellpadding="0" style="border: 1px solid #C9C9C9;">
              ...
              <tr id="billingAddressEntry">
                ...
                <table width="100%" cellspacing="0" cellpadding="2">               
				  ...
				  <tr class="dataTableRow"> 
                    <td class="dataTableContent" valign="middle" align="right" nowrap><?php echo ENTRY_NIP; ?></td>
                    <td colspan="3" valign="top" class="dataTableContent"><input name="update_billing_nip" size="37" value="<?php echo stripslashes($order->billing['nip']); ?>" <?php if (ORDER_EDITOR_USE_AJAX == 'true') { ?>onChange="updateOrdersField('billing_nip', encodeURIComponent(this.value))"<?php } ?>></td>
                  </tr>
                  <tr class="dataTableRow"> 
                    <td class="dataTableContent" valign="middle" align="right" nowrap><?php echo ENTRY_STREET_ADDRESS; ?></td>
				  ...
				  <tr class="dataTableRow"> 
                    <td class="dataTableContent" valign="middle" align="right" nowrap><?php echo ENTRY_STREET_ADDRESS_DOM; ?></td>
                    <td colspan="3" valign="top" class="dataTableContent"><input name="update_billing_street_address_dom" size="37" value="<?php echo stripslashes($order->billing['street_address_dom']); ?>" <?php if (ORDER_EDITOR_USE_AJAX == 'true') { ?>onChange="updateOrdersField('billing_street_address_dom', encodeURIComponent(this.value))"<?php } ?>></td>
                  </tr>
				  <tr class="dataTableRow"> 
                    <td class="dataTableContent" valign="middle" align="right" nowrap><?php echo ENTRY_STREET_ADDRESS_MIESZKANIE; ?></td>
                    <td colspan="3" valign="top" class="dataTableContent"><input name="update_billing_street_address_mieszkanie" size="37" value="<?php echo stripslashes($order->billing['street_address_mieszkanie']); ?>" <?php if (ORDER_EDITOR_USE_AJAX == 'true') { ?>onChange="updateOrdersField('billing_street_address_mieszkanie', encodeURIComponent(this.value))"<?php } ?>></td>
                  </tr>
                  <tr class="dataTableRow"> 
...
<?php
    ...
    for ($i=0, $n=sizeof($shipping_quotes); $i<$n; $i++) {
      for ($j=0, $n2=sizeof($shipping_quotes[$i]['methods']); $j<$n2; $j++) {
		...
		$selected = '';
		$select_ship_now = explode('_', $order->info['shipping_id']);
		if($select_ship_now[0] == $shipping_quotes[$i]['methods'][0]['id']) $selected = ' checked';
        echo '                  <tr class="' . $rowClass . '" onmouseover="rowOverEffect(this)" onmouseout="rowOutEffect(this, \'' . $rowClass . '\')" onClick="selectRowEffect(this, ' . $r . '); setShipping(' . $r . ');">' .
			 ...
			 <script language="JavaScript" type="text/javascript">
                   <!--
                    document.write("<input type=\"radio\" name=\"shipping\" id=\"shipping_radio_' . $r . '\" value=\"' . $shipping_quotes[$i]['id'] . '_' . $shipping_quotes[$i]['methods'][$j]['id'].'\" '.$selected.'>");
	               //-->
                  </script>
