...
//TotalB2B start
        ...
        $customers_groups = tep_db_prepare_input($HTTP_POST_VARS['customers_groups']);
        ...
        $customers = tep_db_prepare_input($HTTP_POST_VARS['customers']);
...
function getTaxRate() {
  var selected_value = document.forms["new_special"].products_tax_class_id.selectedIndex;
  var parameterVal = document.forms["new_special"].products_tax_class_id[selected_value].value;
...
function updateGross(products_price_t) {
  ...
  var grossValue = document.forms["new_special"].elements[products_price_t].value;
  ...
  document.forms["new_special"].elements[products_price_gross_t].value = doRound(grossValue, 4);
}

function updateNet(products_price_t) {
  var taxRate = getTaxRate();
  var products_price_gross_t = products_price_t + "_gross";
  var netValue = document.forms["new_special"].elements[products_price_gross_t].value;

  if (taxRate > 0) {
    netValue = netValue / ((taxRate / 100) + 1);
  }

  document.forms["new_special"].elements[products_price_t].value = doRound(netValue, 4);
}
//TotalB2B end
//--></script>

</head>
<body onload="SetFocus(); updateGross('specials_price')">
        ...
        <td><br><table border="0" cellspacing="0" cellpadding="2">
          ...
          <tr>
            ...
            <td class="main"><?php
				...
				echo tep_draw_input_field('specials_price', (isset($sInfo->specials_new_products_price) ? $sInfo->specials_new_products_price : ''), 'onKeyUp="updateGross(\'specials_price\')"'); 


		      $gross_update = 'updateGross(\'specials_price\');';
		      echo tep_draw_separator('pixel_trans.gif', '24', '15') . '&nbsp;' . tep_draw_pull_down_menu('products_tax_class_id', $tax_class_array, $sInfo->products_tax_class_id, 'onchange="' . $gross_update .'"');
				
				echo tep_draw_input_field('specials_price_gross', (isset($sInfo->specials_new_products_price) ? $sInfo->specials_new_products_price : ''), 'onKeyUp="updateNet(\'specials_price\')"'); 
