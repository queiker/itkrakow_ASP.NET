		...
		if ($_GET['field'] == 'products_quantity') {
			...
			$quantity_difference = ($_GET['new_value'] - $orders_product_info['products_quantity']);
				if (STOCK_LIMITED == 'true'){
					...
					$attrib_q = tep_db_query("select products_id,products_options, products_options_values from orders_products_attributes opa, orders_products op where op.orders_id = '" . $_GET['oID'] . "' AND op.orders_products_id = '" . $_GET['pid'] . "' and opa.orders_products_id = op.orders_products_id");
					 ...
					 */
			         // update the stock
## tylko dla opcji ilosci dla cech
##			         tep_db_query("update ".TABLE_PRODUCTS_STOCK." set products_stock_quantity = products_stock_quantity - ".$quantity_difference . " where products_id=" . $attrib_set['products_id']. " and products_stock_attributes='".$products_stock_attributes."'");
					}
				} else {
					tep_db_query ("UPDATE " . TABLE_PRODUCTS . " SET
					products_ordered = products_ordered + " . $quantity_difference . "
			      ...
			      $order = tep_db_fetch_array($order_query);
			       ...
			       if (STOCK_LIMITED == 'true'){
					...
					while($attrib_set = tep_db_fetch_array($attrib_q))
					  ...
					  */
			          // update the stock
			          tep_db_query("update ".TABLE_PRODUCTS_STOCK." set products_stock_quantity = products_stock_quantity + ".$order['products_quantity'] . " where products_id=" . $attrib_set['products_id']. " and products_stock_attributes='".$products_stock_attributes."'");
					 }
					} else {
					tep_db_query ("UPDATE " . TABLE_PRODUCTS . " SET
					products_ordered = products_ordered - " . $order['products_quantity'] . "
...
<?php
	...
	'      <input type="radio" name="shipping" id="shipping_radio_' . $r . '" value="' . $shipping_quotes[$i]['id'] . '_' . $shipping_quotes[$i]['methods'][$j]['id'].'" '.$selected.'>' . "\n" .
		...
		//Build the email
	   	 $email_order = STORE_NAME . "\n" . 
						...
						if(SEND_CUSTOMER_CONFIRM_ORDER == 'true'){
							$token = substr(md5($_GET['oID']), 2, 7);
							$email_order .= "\n".EMAIL_TEXT_CONFIRM_ORDER . ' ' . tep_catalog_href_link('order_confirm.php', 'order_id=' . $oID.'&tok='.$token, 'SSL') . "\n";
						} else {
							$email_order .= EMAIL_TEXT_INVOICE_URL . ' ' . tep_catalog_href_link(FILENAME_CATALOG_ACCOUNT_HISTORY_INFO, 'order_id=' . (int)$_GET['oID'], 'SSL') . "\n";
						}
                	    $email_order .= EMAIL_TEXT_DATE_MODIFIED . ' ' . strftime(DATE_FORMAT_LONG) . "\n\n";
