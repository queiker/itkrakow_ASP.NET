...
<?php
    if ($error == true) {
      if ($entry_nip_error == true) {
        echo tep_draw_input_field('entry_nip', $account['entry_nip'], 'maxlength="13"') . '&nbsp;' . ENTRY_NIP_ERROR;
      } else {
        echo $account['entry_nip'] . tep_draw_hidden_field('entry_nip');
      }
    } else {
      echo tep_draw_input_field('entry_nip', $account['entry_nip'], 'maxlength="13"');
    }
?></td>
          </tr>
        </table></td>
...
<?php
  if ($is_read_only) {
    echo $account['entry_street_address_dom'];
  } elseif ($error) {
    if ($entry_street_address_dom_error) {
      echo tep_draw_input_field('street_address_dom') . '&nbsp;' . ENTRY_STREET_ADDRESS_DOM_ERROR;
    } else {
      echo $street_address_dom . tep_draw_hidden_field('street_address_dom');
    }
  } else {
    echo tep_draw_input_field('street_address_dom', $account['entry_street_address_dom']) . '&nbsp;' ;
  }
?></td>
          </tr>
          <tr>
...
<?php
  if ($is_read_only) {
    echo $account['entry_street_address_mieszkanie'];
  } elseif ($error) {
//    if ($entry_street_address_mieszkanie_error ||  !tep_not_null($account['entry_street_address_mieszkanie'])) {
    if ($entry_street_address_mieszkanie_error) {
      echo tep_draw_input_field('street_address_mieszkanie') . '&nbsp;' . ENTRY_STREET_ADDRESS_MIESZKANIE_ERROR;
    } else {
      echo $street_address_mieszkanie . tep_draw_hidden_field('street_address_mieszkanie');
    }
  } else {
    echo tep_draw_input_field('street_address_mieszkanie', $account['entry_street_address']) . '&nbsp;' ;
  }
?></td>
          </tr>
<?php
...
<?php
    ...
    if ($is_read_only) {
      ...
      if ($entry_state_error) {
		...
		if(!tep_not_null($country) || $country == 0) $country = 170;
	    $zones_array = array();
		$zones_array[] = array('id' => '', 'text' => '- wybierz województwo z listy -');
...
<?php
  if ($is_read_only) {
    echo $account['customers_cell'];
  } elseif ($processed) {
    echo $cell . tep_draw_hidden_field('cell');
  } else {
    echo tep_draw_input_field('cell', $account['customers_cell']) . '&nbsp;' ;
  }
?></td>
          </tr>
        </table></td>
...
?></td>
          ...
          <tr>
            <td class="main">&nbsp;<?php echo 'Powiadomienia SMS:'; ?></td>
            <td class="main">&nbsp;
<?php
  if ($is_read_only) {
    if ($account['customers_sms_notify'] == '1') {
      echo ENTRY_NEWSLETTER_YES;
    } else {
      echo ENTRY_NEWSLETTER_NO;
    }
  } elseif ($processed) {
    if ($customers_sms_notify == '1') {
      echo ENTRY_NEWSLETTER_YES;
    } else {
      echo ENTRY_NEWSLETTER_NO;
    }
    echo tep_draw_hidden_field('customers_sms_notify');  
  } else {
    echo tep_draw_pull_down_menu('customers_sms_notify', $newsletter_array, $account['customers_sms_notify']) . '&nbsp;' ;
  }
?></td>
          </tr>
		  <tr>
...
<?php
  if ($processed == true) {
	 echo $account['customers_notes'] . tep_draw_hidden_field('customers_notes');
  } else {
	 echo tep_draw_textarea_field('customers_notes', 'soft', '75', '5', $account['customers_notes']);
  }
?></td>
          </tr>
		  <tr>
			...
			<td class="main">&nbsp;
              <select name="customers_discount_sign">
                 <option name="minus" value="-" <?php if (strstr($account['customers_discount'],"-")) echo "selected=\"selected\"" ?>>-</option>
                 <option name="plus" value="+" <?php if (strstr($account['customers_discount'],"+")) echo "selected=\"selected\"" ?>>+</option>
              </select>&nbsp;<?php echo tep_draw_input_field('customers_discount', substr($account['customers_discount'],1,strlen($account['customers_discount'])), 'maxlength="9"'); ?>&nbsp;%
			</td>
		  </tr>
		  <tr>
			...
			?>
			<td class="main">&nbsp;<?php echo tep_draw_pull_down_menu('customers_groups_id', $groups_array, $account['customers_groups_id']); ?></td>
		  </tr>
        </table></td>
      </tr>
    </table></td>
