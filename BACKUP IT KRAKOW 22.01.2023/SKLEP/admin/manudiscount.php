...
<?php
  if ($HTTP_GET_VARS['action'] == 'edit') {
    $manudiscount_query = tep_db_query("select c.manudiscount_id, c.manudiscount_name, c.manudiscount_discount, c.manudiscount_groups_id, c.manudiscount_customers_id, c.manudiscount_manufacturers_id from " . TABLE_MANUDISCOUNT . " c  where c.manudiscount_id = '" . $HTTP_GET_VARS['cID'] . "'");
...
//--></script>
      ...
      <tr>
        <td align="right" class="main"><?php echo tep_image_submit('button_update.gif', IMAGE_UPDATE) . ' <a href="' . tep_href_link('manudiscount.php', tep_get_all_get_params(array('action','cID'))) .'">' . tep_image_button('button_cancel.gif', IMAGE_CANCEL) . '</a>'; ?></td>
      </tr>
      </form>
          ...
          <tr>
            <td valign="top"><table border="0" width="100%" cellspacing="0" cellpadding="2">
               <tr class="dataTableHeadingRow">
			       <td class="dataTableHeadingContent"><?php echo TABLE_HEADING_NAME; ?>&nbsp;<a href="<?php echo "$PHP_SELF?listing=group"; ?>"><?php echo tep_image_button('ic_up.gif');?></a>&nbsp;<a href="<?php echo "$PHP_SELF?listing=group-desc"; ?>"><?php echo tep_image_button('ic_down.gif');?></a></td>
                   <td class="dataTableHeadingContent" align="right"><?php echo TABLE_HEADING_DISCOUNT; ?>&nbsp;<a href="<?php echo "$PHP_SELF?listing=discount"; ?>"><?php echo tep_image_button('ic_up.gif');?></a>&nbsp;<a href="<?php echo "$PHP_SELF?listing=discount-desc"; ?>"><?php echo tep_image_button('ic_down.gif');?></a></td>
...
?>
                ...
                <td class="dataTableContent" align="right"><?php if ( (is_object($cInfo)) && ($manudiscount['manudiscount_id'] == $cInfo->manudiscount_id) ) { echo tep_image(DIR_WS_IMAGES . 'icon_arrow_right.gif', ''); } else { echo '<a href="' . tep_href_link('manudiscount.php', tep_get_all_get_params(array('cID')) . 'cID=' . $manudiscount['manudiscount_id']) . '">' . tep_image(DIR_WS_IMAGES . 'icon_info.gif', IMAGE_ICON_INFO) . '</a>'; } ?>&nbsp;</td>
              </tr>
<?php
    }
?>
              <tr>
                <td colspan="6"><table border="0" width="100%" cellspacing="0" cellpadding="2">
                  <tr>
                    <td class="smallText" valign="top"><?php echo $manudiscount_split->display_count($manudiscount_query_numrows, MAX_DISPLAY_SEARCH_RESULTS, $HTTP_GET_VARS['page'], TEXT_DISPLAY_NUMBER_OF_CUSTOMERS); ?></td>
                    <td class="smallText" align="right"><?php echo $manudiscount_split->display_links($manudiscount_query_numrows, MAX_DISPLAY_SEARCH_RESULTS, MAX_DISPLAY_PAGE_LINKS, $HTTP_GET_VARS['page'], tep_get_all_get_params(array('page', 'info', 'x', 'y', 'cID'))); ?></td>
                  </tr>
<?php
    if (tep_not_null($HTTP_GET_VARS['search'])) {
?>
                  <tr>
                    <td align="right" colspan="2"><?php echo '<a href="' . tep_href_link('manudiscount.php') . '">' . tep_image_button('button_reset.gif', IMAGE_RESET) . '</a>'; ?></td>
                  </tr>
<?php
    } else {
?>
			      <tr>
                    <td align="right" colspan="2" class="smallText"><?php echo '<a href="' . tep_href_link('manudiscount.php', 'page=' . $HTTP_GET_VARS['page'] . '&action=new') . '">' . tep_image_button('button_insert.gif', IMAGE_INSERT) . '</a>'; ?></td>
                  </tr>
<?php
	}
?>
...
<?php
  ...
  switch ($HTTP_GET_VARS['action']) {
    case 'confirm':
            ...
            $contents[] = array('align' => 'center', 'text' => '<br>' . tep_image_submit('button_delete.gif', IMAGE_DELETE) . ' <a href="' . tep_href_link('manudiscount.php', tep_get_all_get_params(array('cID', 'action')) . 'cID=' . $cInfo->manudiscount_id) . '">' . tep_image_button('button_cancel.gif', IMAGE_CANCEL) . '</a>');
      break;
    default:
      if (is_object($cInfo)) {
        $heading[] = array('text' => '<b>' . $cInfo->manudiscount_name . ' </b>');
        $contents[] = array('align' => 'center', 'text' => '<a href="' . tep_href_link('manudiscount.php', tep_get_all_get_params(array('cID', 'action')) . 'cID=' . $cInfo->manudiscount_id . '&action=edit') . '">' . tep_image_button('button_edit.gif', IMAGE_EDIT) . '</a> <a href="' . tep_href_link('manudiscount.php', tep_get_all_get_params(array('cID', 'action')) . 'cID=' . $cInfo->manudiscount_id . '&action=confirm') . '">' . tep_image_button('button_delete.gif', IMAGE_DELETE) . '</a>');
