    ...
    function shipping_select($parameters, $selected = '') {
		...
		for ($i=0, $n=sizeof($shipping_quotes); $i<$n; $i++) {
			for ($j=0, $n2=sizeof($shipping_quotes[$i]['methods']); $j<$n2; $j++) {
		        $select_string .= '<option value="' . $shipping_quotes[$i]['id'] . '"';
		        if ($selected == $shipping_quotes[$i]['module']) $select_string .= ' SELECTED';
		        $select_string .= '>' . $shipping_quotes[$i]['module'] . '</option>';
