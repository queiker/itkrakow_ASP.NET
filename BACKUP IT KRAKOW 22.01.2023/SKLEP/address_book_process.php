...
/*
  ...
  v BF - customer_country_id in addressbook
...
// error checking when updating or adding an entry
  $process = false;
  if (isset($HTTP_POST_VARS['action']) && (($HTTP_POST_VARS['action'] == 'process') || ($HTTP_POST_VARS['action'] == 'update') || ($HTTP_POST_VARS['action'] == 'refresh'))) {
...
// reregister session variables
  ...
  if (isset($HTTP_GET_VARS['edit']) && is_numeric($HTTP_GET_VARS['edit'])) {
    $entry_query = tep_db_query("select entry_gender, entry_company, entry_nip, entry_firstname, entry_lastname, entry_street_address, entry_street_address_dom, entry_street_address_mieszkanie, entry_suburb, entry_postcode, entry_city, entry_state, entry_zone_id, entry_country_id from " . TABLE_ADDRESS_BOOK . " where customers_id = '" . (int)$customer_id . "' and address_book_id = '" . (int)$HTTP_GET_VARS['edit'] . "'");
...
<body <?php if ($refresh) {echo ' onLoad="document.addressbook.state.focus();"';}?>>
<!-- header //-->
...
<?php 
		if (isset($HTTP_GET_VARS['edit'])) { 
			$TITLE = HEADING_TITLE_MODIFY_ENTRY;
		} elseif (isset($HTTP_GET_VARS['delete'])) { 
			$TITLE = HEADING_TITLE_DELETE_ENTRY; 
		} else { 
			$TITLE = HEADING_TITLE_ADD_ENTRY; 
		}
?>
